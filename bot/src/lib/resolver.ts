...
import { getAllCategories } from "./redis.js";
...
export const PROMPT_USER_CONTENT = (userInfo: UserInfo) => {
  ...
  let prompt = `
User context: 
- Call the user by their name or domain, in case they have one or just refer to them as  "user"
...
export const PROMPT_ALL_CATEGORIES = async () => {
  const categories = await getAllCategories();
  let prompt = `
Category context: 
- The available categories: ${JSON.stringify(categories)}
- NOTE: the list of available categories may change from the first message. Use the latest result of Category Context for the current valid list.
`;
